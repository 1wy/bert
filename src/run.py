...
def run():
        ...
        def insert_score():
                ...
                df_t = pd.read_csv('../output/time.csv',dtype={'DATE':str})
                df = df.merge(df_t,on='ID')
                df = add_tradedate(df)
                conn = connect(host='10.24.224.249', port=3306, database='wind', user='wy', password=',.,.,l',charset='utf8')

                cur = conn.cursor()
                for i in range(len(df)):
                        sql = "update FinancialNews set `SCORE`=%f,`TRADE_DT`=\'%s\' where ID=\'%d\'" % (df.iloc[i]['SCORE'], df.iloc[i]['TRADE_DT'],df.iloc[i]['ID'])
                        cur.execute(sql)
                cur.close()
                conn.close()
